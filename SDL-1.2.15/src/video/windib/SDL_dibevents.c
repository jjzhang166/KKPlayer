...
void ancd()
{
	SDL_ParentWindow=CreateWindow(SDL_Appname, SDL_Appname,
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, CW_USEDEFAULT, 0, 0,NULL, NULL, SDL_Instance, NULL);
	printf("SDL_ParentWindow:%d",(int)SDL_ParentWindow);
}
...
printf("DIB_CreateWindow \n");
	...
	if ( SDL_windowid ) {
		...
		SDL_Window = CreateWindow(SDL_Appname, SDL_Appname,
                        SDL_WindowStyle,
                        CW_USEDEFAULT, CW_USEDEFAULT, 0, 0,SDL_ParentWindow, NULL, SDL_Instance, NULL);
		if ( SDL_Window == NULL ) {
		...
		printf("DIB_CreateWindow 2\n");
	}


	if(no_win_border)
	{
		style =GetWindowLong(SDL_Window, GWL_EXSTYLE); //获得窗体扩展风格
		style &= ~WS_EX_APPWINDOW&   ~WS_EX_TOOLWINDOW;; //取消WS_EX_APPWINDOW
		SetWindowLong(SDL_Window, GWL_EXSTYLE, style); //设置窗体扩展风格

	  
		style = GetWindowLong(SDL_Window,GWL_STYLE);//获得窗口风格   
		style =style &~WS_CAPTION & ~WS_SIZEBOX&~WS_SYSMENU&~WS_MINIMIZEBOX;//窗口全屏显示且不可改变大小   
		SetWindowLong(SDL_Window,GWL_STYLE,style); //设置窗口风格   
	}
	/* JC 14 Mar 2006
